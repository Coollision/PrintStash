...
its preview is pending or has failed.
...
extraction, capture covers and similarity work
...
budget and return exact face, edge or vertex projections. The installed native kernel is required; Python reference implementations
remain confined to validation fixtures.
