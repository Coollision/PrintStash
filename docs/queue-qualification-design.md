...
The combined candidate graph also passes an offline, lockfile-enforced all-targets check on PrintStash's production Rust 1.91.0 toolchain. Local exploratory runs used stable 1.98.1 only after that production-toolchain compatibility check; M01 CI and evidence must use 1.91.0.
