...
def test_worker_count_respects_resources(monkeypatch, requested, cpus, mib, expected):
...
def test_effective_cpu_capacity(monkeypatch, files, expected):
...
def test_reserves_memory_across_callers():
...
def test_unknown_memory_uses_conservative_budget(monkeypatch):
...
def test_legacy_render_waits_for_import_reservation(monkeypatch):
...
def test_unknown_mesh_cost_reserves_entire_budget(monkeypatch, tmp_path):
    from app.modules.media import mesh_processing
...
def test_known_mesh_cost_includes_fixed_overhead(monkeypatch, tmp_path):
    from app.modules.media import mesh_processing
...
def test_unavailable_cpu_information_falls_back_to_one(monkeypatch):
