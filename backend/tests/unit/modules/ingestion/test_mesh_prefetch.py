...
def test_full_budget_files_are_consumed_without_deadlock(parallel, monkeypatch):
    monkeypatch.setattr(rb, "estimate_work", lambda *args: 1024 * rb.MIB)
    monkeypatch.setattr(
        mesh_operations, "analyze_mesh", lambda *a, **k: ({}, b"preview")
    )
...
def test_failed_submission_releases_reservation(parallel):
    with PreparedImports(parallel, lambda p: None) as prepared:
...
def test_unreadable_estimate_uses_conservative_admission(parallel, monkeypatch):
...
def test_replays_bounded_progress_labels(parallel, monkeypatch):
    def analyze(path, report, **kwargs):
        for label in (
            "loading_mesh",
            "loading_mesh",
