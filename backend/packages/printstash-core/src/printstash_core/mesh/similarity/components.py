...
def split_components(
    ...
    *,
    ...
    label = getattr(kernel(), "component_labels", None) if native else None
    if label is not None:
        labels = np.frombuffer(label(tris.tobytes(), len(verts)), dtype=np.int64)
    else:
        ...
        neighbors = np.flatnonzero(
            np.all(edges[order[1:]] == edges[order[:-1]], axis=1)
        )
