...
from printstash_core.mesh import native_geometry, native_rasterizer
...
class TestMeasureMeshFallback:
    def test_missing_extension_uses_legacy_measurements(self, monkeypatch):
        monkeypatch.setattr(native_geometry, "kernel", lambda: None)

        assert native_geometry.measure_mesh(trimesh.creation.box()) is None

    def test_older_extension_uses_legacy_measurements(self, monkeypatch):
        monkeypatch.setattr(native_geometry, "kernel", lambda: object())

        assert native_geometry.measure_mesh(trimesh.creation.box()) is None

    def test_custom_mesh_uses_legacy_measurements(self, monkeypatch):
        from types import SimpleNamespace

        monkeypatch.setattr(
            native_geometry,
            "kernel",
            lambda: SimpleNamespace(measure_triangles=object()),
        )

        assert native_geometry.measure_mesh(object()) is None


if hasattr(native_rasterizer.kernel(), "measure_triangles"):
    ...
    class TestMeasureMesh:
        ...
        def test_ignores_unreferenced_vertices_in_bounds(self):
            ...
            assert [
                actual[axis] for axis in ["bbox_x_mm", "bbox_y_mm", "bbox_z_mm"]
            ] == [
                2,
                3,
                4,
