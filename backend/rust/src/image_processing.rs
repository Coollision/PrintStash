...
pub fn process_image<'py>(
    ...
    let count = pixels(source_width, source_height).map_err(PyValueError::new_err)?;
    pixels(width, height).map_err(PyValueError::new_err)?;
    if rgba.len() != count * 4 || !matches!(format, "PNG" | "WEBP") {
        return Err(PyValueError::new_err("invalid image buffer or format"));
    }
    let filter = match filter {
        "lanczos" => FilterType::Lanczos3,
        "bilinear" => FilterType::Bilinear,
        _ => return Err(PyValueError::new_err("invalid resize filter")),
    };
    let output = py
...
pub fn shade_depth<'py>(
    ...
    let count = pixels(width, height).map_err(PyValueError::new_err)?;
    ...
    {
        return Err(PyValueError::new_err("invalid depth image"));
    }
    let data = py.detach(|| {
        let z: Vec<f32> = depth
            .chunks_exact(4)
...
pub fn normalize_thumbnail<'py>(
    ...
    if !(320..=1280).contains(&width) || !margin.is_finite() || !(0.0..0.5).contains(&margin) {
        return Err(PyValueError::new_err("thumbnail_width_invalid"));
    }
