...
impl PreparedPreview {
    ...
    fn new(py: Python<'_>, vertices: &[u8], faces: &[u8], chunk: usize) -> PyResult<Self> {
        if vertices.is_empty()
            ...
            || chunk == 0
        {
            return Err(PyValueError::new_err("invalid mesh preparation buffers"));
        }
        let mut mesh = py
            .detach(|| -> Result<Self, &'static str> {
                ...
                let mut keys: Vec<(i64, u32)> = points
                    ...
                    .map(|(i, p)| {
                        let q = quantize(*p);
                        (
                            ((q[0] - qmin[0]) * span[1] + q[1] - qmin[1]) * span[2] + q[2]
                                - qmin[2],
                            i as u32,
                        )
                    })
        ...
        let (packed, positions) = py.detach(|| {
            (
        ...
        let normals =
            crate::mesh_prepare::smooth_normals(py, &packed, faces, &positions, count, mesh.chunk)?;
        let normal_bytes = normals.as_bytes();
        mesh.normals = py.detach(|| {
            normal_bytes
                .chunks_exact(24)
                .map(|p| {
                    std::array::from_fn(|a| {
                        f64::from_ne_bytes(p[a * 8..a * 8 + 8].try_into().unwrap())
                    })
                })
                .collect()
        });
    ...
    fn render<'py>(
        ...
        {
            return Err(PyValueError::new_err("invalid mesh projection"));
        }
        for a in 0..3 {
            for b in 0..3 {
                ...
                if (dot - if a == b { 1.0 } else { 0.0 }).abs() > 1e-6 {
                    return Err(PyValueError::new_err("invalid mesh rotation"));
                }
            }
        }
        ...
        if (determinant - handedness).abs() > 1e-6 {
            return Err(PyValueError::new_err("invalid mesh handedness"));
        }
        let mut frame = NativeFrame::new(width, height)?;
        let (view, screen) = py.detach(|| {
        ...
        for faces in self.faces.chunks(self.chunk) {
            let (triangles, normals, count) = py.detach(|| {
            ...
            visible += count;
            frame.draw_phong(py, &triangles, 4, &normals, 8, lighting)?;
        }
        if visible == 0 {
            for faces in self.faces.chunks(self.chunk) {
                let triangles = py.detach(|| {
                    faces
                        .iter()
                        .flatten()
                ...
                frame.draw_flat(py, &triangles, 4, flat_color)?;
            }
        }
        frame.rgba(py)
    }
